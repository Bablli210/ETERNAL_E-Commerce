 ...
 * line, name, inspired-by, three notes, price, and two actions.
 */
export function ProductCard({ entry, priority = false, badge, reason, className = "" }: { entry: ScentIndexEntry; priority?: boolean; badge?: string; reason?: string; className?: string }) {
  ...
  return (
    <article className={`group flex flex-col ${className}`}>
      <Link href={`/products/${entry.handle}`} className="relative block" aria-label={entry.title}>
        <ProductImage src={entry.image} alt={entry.title} world={entry.world} priority={priority} className="aspect-[4/5] w-full" />
