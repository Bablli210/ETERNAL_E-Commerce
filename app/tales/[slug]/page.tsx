...
export default async function TalePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <ImageSlot label={tale.heroArt} dark className="h-[56vh] min-h-[360px] w-full" />
      <article className="wrap grid gap-12 py-12 lg:grid-cols-[1fr_360px] lg:gap-20 lg:py-20">
        <div className="mx-auto w-full max-w-[640px] lg:mx-0">
          ...
          <blockquote className="signature mt-12 border-l border-gold pl-6 text-ash">“{tale.signature}”</blockquote>
        ...
        {entry && scent && (
          <aside className="lg:sticky lg:top-28 lg:self-start" aria-label="The scent in this tale">
            <div className="border border-dune bg-paper p-5">
              ...
              <div className="mt-4 flex flex-col gap-2">
                {entry.bottle && <AddToBagButton variant={entry.bottle} product={{ handle: entry.handle, title: entry.title, image: entry.image, lineLabel: entry.lineLabel, world: entry.world }} label="Add to bag" block />}
                {entry.sample && <AddToBagButton variant={entry.sample} product={{ handle: entry.handle, title: entry.title, image: entry.image, lineLabel: entry.lineLabel, world: entry.world }} kind="sample" look="secondary" label={`Try ${entry.sample.label}`} block />}
      ...
      <section className="border-t border-dune">
        <div className="wrap grid gap-10 py-16 lg:grid-cols-[2fr_1fr] lg:py-24">
              ...
              {others.map((t) => (
                <li key={t.slug}>
                  <Link href={`/tales/${t.slug}`} className="group flex flex-col">
                    <ImageSlot label={t.heroArt} className="aspect-[16/10] w-full" />
          ...
          <div className="flex flex-col justify-end bg-sand/60 p-8">
